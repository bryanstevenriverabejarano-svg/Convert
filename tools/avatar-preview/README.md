...
La vista utiliza el PNG original, el relleno oculto, polígonos y pivotes de `app/src/main/assets/avatar/rig.json`. La malla WebGL reproduce las mismas fórmulas que `AvatarRig.deform`; Canvas2D recorta y articula las mismas partes. No usa recursos web externos. No es una ejecución de Android ni una medición de su rendimiento.

`motion-frames.json` se exporta desde `AvatarMotion`, el director de producción. Contiene un escenario programado de saludo, escucha y explicación, sin modelo ni audio reales. Para actualizarlo tras cambiar el director, usa un JDK y una carpeta temporal de clases:

```sh
mkdir -p /tmp/salve-preview-classes
javac --release 11 -d /tmp/salve-preview-classes app/src/main/java/salve/avatar/AvatarMotion.java app/src/main/java/salve/avatar/AvatarMotionProtocol.java tools/avatar-preview/ExportMotionFrames.java
